...
function cyclist#opts_name_get() abort
  echo '[' . join(sort(map(items(s:listchar_opts), {k, v -> printf('%s', v[0])})), ', ') . ']'
endfunction

function cyclist#activate_listchars(...) abort
  " Just make sure list is set, in case people don't do it themselves.
  set list

  let l:opt_name = get(a:, 1, s:current_listchar_opt)

  let listchar_mapped = join(map(
        \ items(filter(s:listchar_opts[l:opt_name], {k, v -> v != ''})),
        \ { opt_name, key -> printf("%s:%s", key[0], key[1])}
        \ ), ',')
  if s:DEBUG
    echo "Listchar set: " . l:opt_name
    echo 'listchar_mapped:'
    echo listchar_mapped
  endif

  let s:current_listchar_opt = l:opt_name
  let &listchars = listchar_mapped
endfunction

function s:cyclist_move_listchars(change) abort
...
function cyclist#next_listchars() abort
  call s:cyclist_move_listchars(1)
endfunction

function cyclist#prev_listchars() abort
  call s:cyclist_move_listchars(-1)
endfunction
